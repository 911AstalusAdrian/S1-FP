...
class UI:
    ...
    def sort_ui(self):
        lists = ['students', 'assignments', 'grades']
        list_to_sort = input("Choose a list to sort (students, assignments or grades): ")
        list_to_sort = list_to_sort.lower()
        if list_to_sort not in lists:
            print("Invalid list")
            return
        sort_criteria = input("Choose a sorting criteria for the list: ")
        sort_criteria = sort_criteria.lower()
        if list_to_sort == 'students':
            sorted_list = self._student_service.sort_students(sort_criteria)
            self._student_service.reorder_students(sorted_list)
        elif list_to_sort == 'assignments':
            sorted_list = self._assignment_service.sort_assignments(sort_criteria)
            self._assignment_service.reorder_assignments(sorted_list)
        else:
            sorted_list = self._grade_service.sort_grades(sort_criteria)
            self._grade_service.reorder_grades(sorted_list)


    def filter_ui(self):
        lists = ['students', 'assignments', 'grades']
        list_to_filter = input("Choose a list to sort (students, assignments or grades): ")
        list_to_filter = list_to_filter.lower()
        if list_to_filter not in lists:
            print("Invalid list")
            return
        filter_criteria = input("Choose a filter criteria: ")
        filter_criteria.strip()
        if filter_criteria == 'deadline':
            year_value = int(input("Choose a year: "))
            month_value = int(input("Choose a month: "))
            day_value = int(input("Choose a day: "))
            filter_value = date(year_value, month_value, day_value)
        else:
            filter_value = input("Choose a filter value:")
            filter_value.strip()
        if list_to_filter == 'students':
            filtered_list = self._student_service.filter_students(filter_criteria, filter_value)
        elif list_to_filter == 'assignments':
            filtered_list = self._assignment_service.filter_assignments(filter_criteria, filter_value)
        else:
            filtered_list = self._grade_service.filter_grades(filter_criteria, filter_value)
        if len(filtered_list) == 0:
            print("There are no values for this filter!")
        else:
