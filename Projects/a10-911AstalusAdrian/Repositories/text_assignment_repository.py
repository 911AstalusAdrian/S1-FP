...
class TextAssignmentRepository(AssignmentRepository):
    ...
    def update_assignment(self, id_, new_deadline_):
        previous_deadline = super().update_assignment(id_, new_deadline_)
        self._save()
    ...
    def _save(self):
        ...
        for assignment in self._assignment_data:
            line = assignment.id + ';' + assignment.description + ';' + str(assignment.deadline)
            file.write(line)
            file.write('\n')
        file.close()
